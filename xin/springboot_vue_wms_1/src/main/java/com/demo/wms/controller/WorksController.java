...
public class WorksController {
    ...
    public Result save(@RequestBody Works works) {
        ...
        if (null != works.getId()) {
            fileService.remove(Wrappers.lambdaQuery(FileEntity.class)
                    .eq(FileEntity::getBusId, works.getId()));
        }
        worksService.saveOrUpdate(works);

        List<FileEntity> files = works.getFiles();
        files.forEach(i -> i.setBusId(works.getId()));
        fileService.saveBatch(files);

        return Result.suc();//判断是否保存保存的话返回suc,否则返回fail
    }
    //更新
...
//        IPage result= CourseService.pageC(page);//返回模糊查询结果
        IPage<Works> result = worksService.pageCC(page, lambdaQueryWrapper);//返回模糊查询结果

        result.getRecords().forEach(i -> {
            i.setFiles(fileService.list(Wrappers.lambdaQuery(FileEntity.class)
                    .eq(FileEntity::getBusId, i.getId())));
        });
